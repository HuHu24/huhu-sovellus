...
export async function POST(request: NextRequest) {
  ...
  const decodedClaims = await getDecodedClaims(cookies().get("session")?.value || "")
  ...
  await auth().setCustomUserClaims(decodedClaims.uid, { subcamp: result.subcamp })
...
function errorResponse() {
  return NextResponse.json(
      {
        message: "Error occurred when processing custom claim. Check request body formatting and data",
      },
      { status: 400 }
  )
}
